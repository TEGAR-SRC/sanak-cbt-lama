...
class Tes_kerjakan extends Tes_Controller {
    ...
    public function index($tes_id=null){
        if(!empty($tes_id)){
            ...
            if($query_tes->num_rows()>0){
                ...
                // Perbolehkan status 1 (aktif) dan 3 (berlangsung)
                if($query_tes->tesuser_status == 4){
                    // Jika status sudah selesai/terkunci, redirect ke dashboard
                    $this->session->set_flashdata('error', 'Tes sudah dihentikan, Anda tidak bisa melanjutkan.');
                    redirect('tes_dashboard');
                }
                if($query_tes->tesuser_status != 3 && $query_tes->tesuser_status != 1){
                    redirect('tes_dashboard');
                }
    ...
    function hentikan_tes(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if(!empty($centang)){
                ...
                $status['tesuser_id'] = $tesuser_id; // Tambahkan tesuser_id untuk redirect
            }else{
                $status['status'] = 0;
    ...
    /**
     * Endpoint untuk mengakhiri ujian secara otomatis (tanpa centang),
     * dipanggil oleh script JS saat browser/tab ditutup
     */
    public function selesai_ujian_otomatis() {
        $tes_id = $this->input->post('tes_id', TRUE);
        $tes_user_id = $this->input->post('tes_user_id', TRUE);
        $status = ['status' => 0, 'pesan' => 'Data tidak lengkap'];
        if(!empty($tes_id) && !empty($tes_user_id)) {
            $data_tes['tesuser_status'] = 4; // status selesai
            $this->cbt_tes_user_model->update('tesuser_id', $tes_user_id, $data_tes);
            $status = ['status' => 1, 'pesan' => 'Tes otomatis dihentikan', 'tesuser_id' => $tes_user_id];
        }
        echo json_encode($status);
    }

    public function cek_status_tesuser() {
        $tes_user_id = $this->input->post('tes_user_id');
        $status = 0;
        if (!empty($tes_user_id)) {
            $query = $this->cbt_tes_user_model->get_by_kolom_limit('tesuser_id', $tes_user_id, 1);
            if ($query->num_rows() > 0) {
                $row = $query->row();
