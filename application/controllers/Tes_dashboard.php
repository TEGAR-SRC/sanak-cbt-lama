    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			if($this->cbt_tes_topik_set_model->count_by_kolom('tset_tes_id', $temp->tes_id)->row()->hasil>0){
	            ...
	            $query_tes_user = $this->cbt_tes_user_model->get_by_user_tes($user_id, $temp->tes_id);
	            if($query_tes_user->num_rows()>0){
	                $query_test_user = $query_tes_user->row();
	                if($query_test_user->tesuser_status != 4){
	                    // Jika masih dalam pengerjaan, bisa lanjutkan
	                    $tanggal = new DateTime();
	                    $tanggal_tes = new DateTime($query_test_user->tesuser_creation_time);
	                    $tanggal_tes->modify('+'.$temp->tes_duration_time.' minutes');
	                    if($tanggal<$tanggal_tes){
	                        $record[] = 'Sedang Berlangsung';
	                        $record[] = '0'; // Nilai belum tersedia
	                        $record[] = '<a href="'.site_url().'/tes_kerjakan/index/'.$temp->tes_id.'" style="cursor: pointer;" class="btn btn-default btn-xs">Lanjutkan</a>';
	                    }else{
	                        // Sudah selesai waktunya, tidak tampilkan tombol apapun
	                        $record[] = 'Selesai';
	                        $record[] = '0'; // Nilai belum tersedia
	                        $record[] = '';
	                    }
	                }else{
	                    // Sudah pernah mengerjakan dan selesai, tampilkan nilai
	                    $record[] = 'Selesai';
	                    // Ambil nilai dari database
	                    $nilai_query = $this->cbt_tes_soal_model->get_nilai($query_test_user->tesuser_id);
	                    if($nilai_query->num_rows() > 0){
	                        $nilai_data = $nilai_query->row();
	                        $nilai_tampil = $nilai_data->hasil . ' / ' . $temp->tes_max_score;
	                    }else{
	                        $nilai_tampil = '0 / ' . $temp->tes_max_score;
	                    }
	                    $record[] = $nilai_tampil;
	                    
	                    // Cek apakah pengaturan Detail Hasil diaktifkan
	                    if($temp->tes_detail_to_users == 1){
	                        $record[] = '<a href="'.site_url().'/tes_hasil_detail/index/'.$query_test_user->tesuser_id.'" style="cursor: pointer;" class="btn btn-info btn-xs">Lihat Hasil</a>';
	                    }else{
	                        $record[] = '';
	                    }
	                }
	            }else{
	                // Belum pernah mengerjakan, tampilkan tombol Kerjakan
	                $record[] = 'Belum Mulai';
	                $record[] = '-';
