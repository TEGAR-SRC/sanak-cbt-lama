...
class Tes_hasil extends Member_Controller {
    ...
    function export($tes_id=null, $grup_id=null, $waktu=null, $urutkan=null, $status=null, $keterangan=null){
        if(!empty($tes_id) AND !empty($grup_id) AND !empty($waktu) AND !empty($urutkan) AND !empty($status)){
			...
			$excel = $this->spreadsheet->load($inputFileName);
			...
			if($query->num_rows()>0){
				foreach ($query->result() as $temp) {
					...
					// Format tanggal ke lokal (Y-m-d H:i) dan masukkan sebagai string
					$worksheet->setCellValueByColumnAndRow(1, $row, date('Y-m-d H:i', strtotime($temp->tesuser_creation_time)));
					...
					$worksheet->setCellValueByColumnAndRow(6, $row, (float)$temp->nilai);
					$row++;
				}
			} else {
				// Jika kosong, beri info
				$worksheet->setCellValueByColumnAndRow(0, $row, 'Tidak ada data');
			}
			...
			$filename='Data Hasil Tes - '.date('Y-m-d_H-i').'.xlsx'; //save our workbook as this file name
			$this->spreadsheet->workbook = $excel;
			// Explicit output (duplikat method untuk jaga kompatibilitas header di beberapa hosting)
