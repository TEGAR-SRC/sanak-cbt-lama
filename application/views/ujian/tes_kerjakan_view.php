    ...
    <section class="content">
        ...
        <div class="row">
            <div class="box box-success box-solid">
                ...
                <div class="box-footer">
                    <button class="btn btn-default pull-right" id="btn-hentikan">Selesai Ujian</button>
                </div>
            </div><!-- /.box -->
        </div>
    </section><!-- /.content -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <div id="trx-judul">Konfirmasi Selesai Ujian</div>
                </div>
                    ...
                    <div class="row-fluid">
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="hentikan-centang" name="hentikan-centang" value="1"> Centang dan klik tombol untuk Selesaikan ujian.
                                    </label>
                                </div>
                            </div>
                        </div>
				...
				<div class="box-footer">
					<button type="submit" id="tambah-simpan" class="btn btn-primary">Selesai Ujian</button>
...
<script type="text/javascript">
        ...
        $('#form-hentikan').submit(function(){
            ...
            $.ajax({
                    ...
                    success:function(respon){
                        var obj = $.parseJSON(respon);
                        if(obj.status==1){
                            // Redirect ke halaman hasil tes setelah ujian selesai
                            window.location.href = "<?php echo site_url(); ?>/tes_hasil_detail/index/" + obj.tesuser_id;
                        }else{
                            $("#modal-proses").modal('hide');
                            notify_error(obj.pesan);
                        }
    ...
    // Script untuk otomatis mengakhiri ujian jika browser/tab ditutup
    window.addEventListener('beforeunload', function (e) {
        // Kirim request ke backend untuk mengakhiri ujian otomatis
        navigator.sendBeacon(
            '<?= site_url("tes_kerjakan/selesai_ujian_otomatis") ?>',
            new URLSearchParams({
                tes_id: '<?= isset($tes_id) ? $tes_id : '' ?>',
                tes_user_id: '<?= isset($tes_user_id) ? $tes_user_id : '' ?>'
            })
        );
    });
</script>